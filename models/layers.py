...
class PatchTransformerEncoder(tf.keras.Model):
    ...
    def call(self, x):
        ...
        x += self.positional_encodings[:h * w]  # broadcast positional encodings
...
if __name__ == "__main__":
    ...
    # Dummy input: [batch, height, width, channels] => [2, 40, 40, 3]
    dummy_input = tf.random.normal([2, 40, 40, 3])
    ...
    encoded_reshaped = tf.reshape(encoded_output, [2, 4, 4, 128])

    # Create dummy K: [batch, out_channels, embedding_dim]
    K = tf.random.normal([2, 64, 128])
