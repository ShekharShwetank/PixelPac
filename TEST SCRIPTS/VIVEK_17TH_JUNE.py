...
def preprocess_image(img, inputWidth=256, inputHeight=256):
...
if __name__ == "__main__":
    ...
    output_video_path = os.path.join(output_dir, 'aligned_output.mp4')
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # fallback to 'mp4v' if needed
    ...
    while True:
        ...
        # Preprocess
        normalized_img, raw_img_rgb = preprocess_image(frame)
        ...
        # Display output
        cv2.imshow('Real-time Depth Estimation', cv2.cvtColor(combined, cv2.COLOR_RGB2BGR))
    ...
    print(f"✅ High-quality video saved: {output_video_path}")